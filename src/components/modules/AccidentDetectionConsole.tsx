...
type Incident = { id: string; time: string; risk: number; source: "camera" | "video"; reason: string };
type SceneModel = {
...
export function AccidentDetectionConsole() {
  ...
  async function analyze(activeSource: "camera" | "video") {
    ...
    try {
      ...
      let previousGray: Uint8Array | null = null; let previousArea = 0; let lastScan = 0; let riskFrames = 0; let lastAlert = 0; let analyzedFrames = 0;
      ...
      const loop = async (time: number) => {
        ...
        if (video && !video.paused && video.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA && time - lastScan > 550) {
          try {
            ...
            const people = predictions.filter((item) => item.class === "person" && item.score >= 0.45);
            ...
            const vehicleImpactEvidence = currentVehicles.length > 0
              && (motionSpike || areaShift > 0.08)
              && (closePair || accidentProbability > 0.72);
            const fallEvidence = people.length > 0
              && motionSpike
              && (people.some((person) => person.bbox[2] > person.bbox[3] * 0.9) || accidentProbability > 0.8);
            const sceneImpactEvidence = predictions.length > 0 && accidentProbability > 0.92 && motionSpike;
            const contextualEvidence = vehicleImpactEvidence || fallEvidence || sceneImpactEvidence;
            analyzedFrames += 1;
            const warmedUp = analyzedFrames >= 8;
            const validatedModelRisk = contextualEvidence ? Math.round(accidentProbability * 100) : 0;
            const currentRisk = warmedUp && contextualEvidence
              ? Math.min(100, Math.round(
                  validatedModelRisk * 0.82
                  + heuristicRisk * 0.18
                  + (vehicleImpactEvidence || fallEvidence ? 8 : 5),
                ))
              : Math.min(60, heuristicRisk);
            const reasons = [
              contextualEvidence && accidentProbability > 0.9 ? `trained model ${Math.round(accidentProbability * 100)}%` : "",
              vehicleImpactEvidence ? "moving vehicles in collision proximity" : "",
              fallEvidence ? "sudden motion with a horizontal person posture" : "",
              motionSpike ? `sudden motion spike (${Math.round(motion)})` : "",
              currentVehicles.length > 0 && areaShift > 0.08 ? `rapid vehicle-area change (${Math.round(areaShift * 100)}%)` : "",
            ].filter(Boolean);
            const highRisk = warmedUp && contextualEvidence && currentRisk > 90;
            riskFrames = highRisk ? riskFrames + 1 : Math.max(0, riskFrames - 1);
            if (riskFrames >= 2 && time - lastAlert > 30000) {
              ...
              const incident: Incident = {
                ...
                risk: currentRisk,
                source: activeSource,
                reason: reasons.join(", ") || "combined detection signals exceeded the 90% threshold",
              };
            ...
            setVehicles(currentVehicles); setMotionScore(Math.round(motion)); setModelScore(validatedModelRisk); setRiskScore(currentRisk); lastScan = time;
          } catch (cause) {
  ...
  return <section className="space-y-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><Badge variant={alertActive ? "warning" : "success"}>{alertActive ? "Possible accident" : "Monitoring ready"}</Badge><h2 className="mt-4 text-3xl font-semibold text-white">Browser accident detection</h2><p className="mt-2 text-sm text-slate-400">Adapted from the supplied vehicle, motion, proximity, and impact-detection pipeline.</p></div><div className="flex flex-wrap gap-2"><Button className="md:hidden" variant="secondary" onClick={switchCamera}><RefreshCw className="h-4 w-4" />{facingMode === "environment" ? "Use front camera" : "Use rear camera"}</Button><label className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 text-sm font-semibold text-white hover:bg-white/10"><FileVideo className="h-4 w-4" />Analyze video<input className="sr-only" type="file" accept="video/*" onChange={(event) => void openVideo(event)} /></label>{status === "idle" || status === "error" ? <Button onClick={() => void startCamera()}><Camera className="h-4 w-4" />Open camera</Button> : <Button variant="danger" onClick={stop}><StopCircle className="h-4 w-4" />Stop</Button>}</div></div>
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]"><Card className="overflow-hidden p-0"><div className={`relative aspect-video bg-black ${alertActive ? "ring-4 ring-rose-500/80" : ""}`}><video ref={videoRef} muted playsInline className="h-full w-full object-contain" />{vehicles.map((vehicle, index) => { const [x,y,w,h]=vehicle.bbox; return <div key={`${vehicle.class}-${index}`} className="absolute border-2 border-emerald-300" style={{left:`${x/width*100}%`,top:`${y/height*100}%`,width:`${w/width*100}%`,height:`${h/height*100}%`}}><span className="absolute -top-7 left-0 bg-emerald-300 px-2 py-1 font-mono text-xs font-bold text-slate-950">{vehicle.class} {Math.round(vehicle.score*100)}%</span></div>; })}{alertActive ? <div className="absolute inset-0 flex items-center justify-center bg-rose-950/35"><div className="rounded-xl border border-rose-300 bg-rose-600/90 px-6 py-4 text-center shadow-2xl"><AlertTriangle className="mx-auto h-8 w-8" /><p className="mt-2 font-bold uppercase tracking-wider">Possible accident detected</p></div></div> : null}{["requesting","loading"].includes(status) ? <div className="absolute inset-0 flex items-center justify-center bg-slate-950/85"><Loader2 className="h-8 w-8 animate-spin text-emerald-200" /><p className="ml-3 text-sm text-slate-200">{status === "requesting" ? "Waiting for camera permission..." : "Preparing both detection models · video paused..."}</p></div> : null}</div></Card>
      <Card><CardContent><h3 className="font-semibold text-white">Live risk analysis</h3><div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-xl bg-white/5 p-3 text-center"><p className="text-2xl font-semibold text-white">{vehicles.length}</p><p className="text-[10px] uppercase text-slate-500">Vehicles</p></div><div className="rounded-xl bg-white/5 p-3 text-center"><p className="text-2xl font-semibold text-white">{motionScore}</p><p className="text-[10px] uppercase text-slate-500">Motion</p></div><div className="rounded-xl bg-cyan-400/10 p-3 text-center"><p className="text-2xl font-semibold text-white">{modelScore}%</p><p className="text-[10px] uppercase text-slate-500">Validated model</p></div><div className={`rounded-xl p-3 text-center ${riskScore > 90 ? "bg-rose-400/15" : "bg-emerald-400/10"}`}><p className="text-2xl font-semibold text-white">{riskScore}%</p><p className="text-[10px] uppercase text-slate-500">Combined risk</p></div></div><p className="mt-4 text-xs leading-5 text-slate-500">Source: {source}. Analysis first learns the still-scene baseline. A high-confidence model result is then accepted when accompanied by a sudden visual change, detected vehicle impact, or fall posture. Risk must remain above 90% for two analyzed frames.</p>{error ? <p className="mt-4 rounded-lg border border-rose-300/20 bg-rose-400/10 p-3 text-sm text-rose-200">{error}</p> : null}<h3 className="mt-6 font-semibold text-white">Recent incidents</h3><div className="mt-3 space-y-2">{incidents.map((incident) => <div key={incident.id} className="rounded-lg border border-rose-300/15 bg-rose-400/5 p-3 text-sm"><div className="flex items-center justify-between gap-3"><span className="text-slate-300">{incident.time} · {incident.source}</span><Badge variant="warning">Combined risk {incident.risk}%</Badge></div><p className="mt-2 text-xs leading-5 text-rose-100/80">Reason: {incident.reason}</p></div>)}{incidents.length === 0 ? <p className="rounded-lg border border-dashed border-white/10 p-4 text-center text-xs text-slate-500">No validated risk events above 90% in this session.</p> : null}</div></CardContent></Card></div>
  </section>;
}
