...
export function AccidentDetectionConsole() {
  ...
  function stop() {
    ...
    setStatus("idle"); setVehicles([]); setMotionScore(0); setRiskScore(0); setAlertActive(false); setError("");
  }
  ...
  async function analyze(activeSource: "camera" | "video") {
    ...
    try {
      const model = await loadAccidentModel();
      ...
      const loop = async (time: number) => {
        ...
        if (video && !video.paused && video.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA && time - lastScan > 550) {
          try {
            ...
            const currentRisk = Math.min(100, Math.round((closePair ? 42 : 0) + Math.min(35, motion * 1.4) + Math.min(23, areaShift * 30)));
            const possibleImpact = closePair && motionSpike && (areaShift > 0.08 || motion > 20);
            riskFrames = possibleImpact ? riskFrames + 1 : Math.max(0, riskFrames - 1);
            ...
            setVehicles(currentVehicles); setMotionScore(Math.round(motion)); setRiskScore(currentRisk); lastScan = time;
          } catch (cause) {
