      ...
      const loop = async (time: number) => {
        ...
        if (video && !video.paused && video.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA && time - lastScan > 550) {
          try {
            ...
            const vehicleImpactEvidence = currentVehicles.length >= 2 && closePair && (motionSpike || areaShift > 0.12);
            const fallEvidence = people.some((person) => person.bbox[2] > person.bbox[3] * 1.15) && motionSpike;
            const contextualEvidence = vehicleImpactEvidence || fallEvidence;
            analyzedFrames += 1;
            const warmedUp = analyzedFrames >= 10;
            ...
            const currentRisk = warmedUp && contextualEvidence
              ? Math.min(100, Math.round(validatedModelRisk * 0.72 + heuristicRisk * 0.28))
            ...
            const reasons = [
              ...
              currentVehicles.length > 0 && areaShift > 0.12 ? `rapid vehicle-area change (${Math.round(areaShift * 100)}%)` : "",
            ].filter(Boolean);
            const highRisk = warmedUp && contextualEvidence && currentRisk > 90;
            riskFrames = highRisk ? riskFrames + 1 : Math.max(0, riskFrames - 1);
            if (riskFrames >= 4 && time - lastAlert > 30000) {
