...
import { ArrowLeft, Camera, Check, Crop, Database, ImagePlus, Loader2, Pencil, Trash2, X } from "lucide-react";
import { Link, Navigate } from "react-router-dom";
import { Badge } from "@/components/ui/Badge";
import { Button } from "@/components/ui/Button";
import { Card, CardContent } from "@/components/ui/Card";
import { Input } from "@/components/ui/Input";
import { cropImage, extractEmbedding, imageFromDataUrl, normalizeObjectType } from "@/lib/ownerIdentification";
...
export function AssetRegistrationPage() {
  ...
  function capture() {
    ...
    setImages((current) => [...current, canvas.toDataURL("image/jpeg", 0.85)]);
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const embeddings = await Promise.all(images.map(async (source, index) => source === oldImages[index] && oldEmbeddings[index]?.length ? oldEmbeddings[index] : extractEmbedding(await imageFromDataUrl(source))));
      const record = { category: values.category, ownerName: values.name, employeeId: "", department: "", objectType: normalizeObjectType(values.objectType), assetName: values.name, assetId: values.identityId, image: images[0], images, embedding: embeddings[0], embeddings };
      if (editingId) updateAsset(editingId, record); else addAsset(record);
      resetForm();
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-12">
    ...
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      <Card><CardContent><h2 className="text-xl font-semibold text-white">Reference images</h2><div className="mt-5 overflow-hidden rounded-xl border border-white/10 bg-black/40"><video ref={videoRef} className="aspect-video w-full object-cover" muted autoPlay playsInline /></div><div className="mt-4 flex flex-wrap gap-3"><Button onClick={cameraOn ? capture : startCamera}><Camera className="h-4 w-4" />{cameraOn ? "Capture another" : "Open webcam"}</Button><label className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-lg border border-borderSubtle bg-white/5 px-4 text-sm font-semibold text-slate-100 hover:bg-white/10"><ImagePlus className="h-4 w-4" />Upload images<input multiple type="file" accept="image/*" className="sr-only" onChange={(event) => { for (const file of Array.from(event.target.files ?? [])) { const reader = new FileReader(); reader.onload = () => setImages((current) => [...current, String(reader.result)]); reader.readAsDataURL(file); } event.target.value = ""; }} /></label></div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{images.map((image, index) => <div key={`${image.slice(-20)}-${index}`} className="relative overflow-hidden rounded-xl border border-white/10"><img src={image} alt={`Reference ${index + 1}`} className="aspect-square w-full object-cover" /><div className="absolute inset-x-1 bottom-1 flex justify-between"><Button size="sm" variant="secondary" onClick={() => setCropIndex(index)}><Crop className="h-3.5 w-3.5" />Crop</Button><Button size="sm" variant="danger" aria-label={`Remove image ${index + 1}`} onClick={() => setImages((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 className="h-3.5 w-3.5" /></Button></div></div>)}</div>{images.length ? <p className="mt-3 text-xs text-slate-500">{images.length} reference image{images.length === 1 ? "" : "s"} selected. Add different angles for stronger matching.</p> : null}</CardContent></Card>
    ...
    <Card><CardContent><div className="flex items-center justify-between"><h2 className="text-xl font-semibold text-white">Registered assets and people</h2><Badge variant="success">{assets.length} enrolled</Badge></div><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{assets.map((asset) => <article key={asset.id} className="overflow-hidden rounded-xl border border-white/10 bg-white/[0.025]">{asset.image ? <img src={asset.image} alt={asset.assetName} className="h-36 w-full object-cover" /> : null}<div className="p-4"><div className="flex justify-between gap-3"><div><Badge variant="neutral">{asset.category ?? (asset.objectType === "person" ? "person" : "asset")}</Badge><p className="mt-2 font-semibold text-white">{asset.assetName}</p></div><div className="flex"><Button variant="ghost" size="sm" aria-label={`Edit ${asset.assetName}`} onClick={() => edit(asset.id)}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="sm" aria-label={`Remove ${asset.assetName}`} onClick={() => removeAsset(asset.id)}><Trash2 className="h-4 w-4" /></Button></div></div><p className="mt-3 text-xs text-slate-500">{asset.assetId} · {asset.objectType}</p><p className="mt-2 text-xs text-slate-500">{asset.embeddings?.length ?? (asset.embedding ? 1 : 0)} visual reference(s)</p></div></article>)}</div>{assets.length === 0 ? <p className="mt-5 rounded-xl border border-dashed border-white/10 p-8 text-center text-slate-500">No assets or people registered yet.</p> : null}</CardContent></Card>
